/**
 * Task-model-map business.
 *
 * Its only job: which business task uses which catalog model. Links
 * (`app_model_task_config`) map a task id to a catalog model `identifier`;
 * the identifier must resolve in the `model-providers` catalog and cover the
