...
- **Skip** when `image_content_hash`, `caption_model`, and `caption_prompt_version` all match the stored row and a description exists.
...
`caption_model` and the caption task are application wiring: the `md-image-captioning` task link in `app_model_task_config`. The `embedding` task link drives `embedding_model`.
