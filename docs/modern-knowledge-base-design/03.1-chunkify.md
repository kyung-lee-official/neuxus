...
No I/O. Caller supplies `body` (frontmatter already stripped) and resolved knobs; an idempotent `normalizeBody` pass runs first, and each `text` is `body.slice(start, end)` into the normalized string — keep blank lines inside the span; do not compact.
...
**Tree:** page →\* parent →\* child. Children never cross a parent boundary; overlap (if any) stays inside one parent. Query expand: [04-retrieval.md](./04-retrieval.md).
...
It does not decide parent or child size. It only classifies source regions so later packing can work on whole blocks. Atomic blocks win over paragraph rules; interior of an unclosed fence is not re-classified as headings or lists.

Block kinds and edge rules: [Atomic blocks](#atomic-blocks-and-edge-cases).

## Packing

At query time a hit on a child loads its parent via the link already stored. Packing builds those linked units from lexed blocks.
...
Packing is the middle path: concatenate neighboring blocks toward `childTargetTokens` / `parentMaxTokens`, stop before the next unit would exceed the target (mixed packs).
...
DB may store overrides; missing values use these **application defaults**:
...
Fixed policy (not a knob): parent cuts `##`, then `###`, then block packs. Atomic blocks may exceed size knobs as one unit ([Oversized](#oversized-atomic-blocks)).
...
`childOverlapTokens` is a **budget**, used only after a [forced prose split](#forced-prose-splits). Measure it on the previous piece’s **tail after the snap**, not as a raw N-token suffix of the unsplit paragraph. The resulting overlap may be under `childOverlapTokens`.
...
1. One parent per `##` (heading through next `##` / EOF). Preamble before the first `##` is its own first parent. No `##` means one parent for the whole body. Trailing blank runs attach to the **previous** parent (and its last child): parent 0 ends at the last `\n` before the next `##`.
2. If over `parentMaxTokens`: split on `###`, then pack blocks ≤ max without breaking atomics.
...
2. [Fence intro](#fence-intro-glue) stays in one child (glue wins over knobs).
...
Same normalized `body` + knobs ⇒ same spans. Every region is exactly one block type.
...
- Unclosed runs through EOF; interior lines are not re-lexed as headings/lists.
...
A line that starts with indent ≤3 and `<`, except image (`<img>`). Span through the next blank line or EOF. Interior lines are not re-lexed as headings, lists, or fences.

### Forced prose splits

Only when a **single paragraph** exceeds `childHardMaxTokens`. Choose the cut end, then the next start, with the [overlap](#overlap) rules. Never inside fences, tables, or headings.

### Oversized atomic blocks

Atomics and [fence-intro](#fence-intro-glue) pairs are never sliced to satisfy knobs.

| Case                                        | Parent          | Child     |
| ------------------------------------------- | --------------- | --------- |
| ≤ `parentMaxTokens`, > `childHardMaxTokens` | Current section | Alone     |
| > `parentMaxTokens`                         | Block alone     | Same text |

### Empty body

Whitespace-only yields no parents and no children.
