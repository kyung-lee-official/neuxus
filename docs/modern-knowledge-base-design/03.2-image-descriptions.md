...
`caption_model` and the caption task are application wiring: the `md-image-captioning` task link in `app_model_task_config`. The `embedding` task link drives `embedding_model`. Both identifiers are `{providerId}::{modelId}`.
