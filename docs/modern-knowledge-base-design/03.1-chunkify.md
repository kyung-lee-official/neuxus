...
It does not decide parent or child size. It only classifies source regions so later packing can work on whole blocks. Fences win over paragraph rules; interior of an unclosed fence is not re-classified as headings or lists.
...
Same normalized `body` + knobs ⇒ same spans. Every region is exactly one block type; fences win over paragraph rules.
