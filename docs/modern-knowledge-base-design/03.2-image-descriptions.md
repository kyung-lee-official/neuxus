...
Ingest records the policy ([02-ingest.md](./02-ingest.md#image-policy-reconciliation)). This doc is the **image contract**: sidecar format, the caption pass, description vectors, and how descriptions are used at retrieval/synthesis time. Chunkify: [03.1-chunkify.md](./03.1-chunkify.md). Embed utility: [embed.md](./embed.md). Retrieval: [04-retrieval.md](./04-retrieval.md). Synthesis: [05-synthesis.md](./05-synthesis.md). Schema: [appendix-a-data-model.md](./appendix-a-data-model.md#tables-postgresql).

```text
page.md + page.meta.yaml
  → ingest: policy per image (ignore | manual | vision-captioning)
  → caption pass: vision model → description (vision-captioning only)
  → embed utility: description → vector
  → synthesis: image syntax replaced by description (ephemeral)
```

## Sidecar format

For every ingested `<page>.md`, the walker looks for a sibling `<page>.meta.yaml` in the same directory ([01-corpus.md](./01-corpus.md#include-and-exclude)). It is optional; when absent, every image uses the default policy. It is **not** a page and never gets a `source_path` / `slug`.
...
- **Default policy for an unlisted image is `vision-captioning`.** This preserves the behavior where every referenced image is described for retrieval. Set a sidecar entry to `ignore` to opt out.
...
`vision-captioning` is the sidecar policy name; it selects the catalog model linked to the **`md-image-captioning`** task ([Models](#models)).
...
| `caption_prompt_version` | integer version of the hardcoded caption prompt (`null` for `manual`)                           |
...
flowchart TD
  ...
  Select --> For{For each row?}
  For -- no --> Done([done])
  For -- yes --> Fresh{"image_content_hash,<br/>caption_model,<br/>caption_prompt_version<br/>all match?"}
  Fresh -- yes --> Next[Next row]
  Fresh -- no --> Read[Read image bytes]
  Read --> Hash["image_content_hash = sha256(bytes)"]
  Hash --> Vision["vision model<br/>(md-image-captioning task)"]
  Vision --> Store["UPDATE description, caption_model,<br/>caption_prompt_version, description_hash<br/>clear embedding"]
  Store --> Next
  Next --> For
```

- **Skip** when `image_content_hash`, `caption_model`, and `caption_prompt_version` all match the stored row and a description exists. Image bytes, caption model, and prompt text are the full freshness key.
...
- `caption_prompt_version` is a code constant. Bump it when the prompt text changes so captions are regenerated.

No description text is ever written into `kb_pages.body`.

## Description embedding

The description is embedded with the shared embed utility ([embed.md](./embed.md)) using the **`embedding`** task model. Embed a description when:
...
This is a **second retrieval channel**, separate from the child-vector channel ([04-retrieval.md](./04-retrieval.md#image-description-channel)). It surfaces image descriptions that match the question even when the surrounding page text does not. Its hits are capped and merged into the synthesis context, labelled with the page slug/title.

## Transitions

| Change                                             | Effect                                                                                                                          |
| -------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| `vision-captioning` → `manual`                     | Ingest writes the sidecar description; clears `caption_model` / `caption_prompt_version`; caption pass no longer selects the row |
| `manual` → `vision-captioning`                     | Ingest clears the manual description; caption pass re-captions and clears the embedding                                          |
| any → `ignore`                                      | Row keeps `policy = ignore` with `description = null`, `embedding = null`; excluded from search                                  |
| image bytes change                                 | `image_content_hash` differs; caption pass re-captions (vision) or ingest refreshes the hash (manual)                            |
| image no longer referenced by the page             | Ingest deletes the row (and its vector)                                                                                          |
| `embedding` task model changes                     | Embed utility re-embeds descriptions whose `embedding_model` differs                                                             |

## Removed syntax

The old `<!-- image_desc --> … <!-- /image_desc -->` in-body markers are **no longer recognized**. They are ordinary HTML comments and stay in the body (chunkify treats them as an HTML block). To migrate a corpus, move each manual description into the sibling sidecar and delete the comment pair. Do not keep both mechanisms.

## Models

`caption_model` and the caption task are application wiring: the `md-image-captioning` task link in `app_model_task_config` ([appendix-a-data-model.md](./appendix-a-data-model.md)). The `embedding` task link drives `embedding_model`. Both identifiers are `{providerId}::{modelId}`. See [embed.md](./embed.md).
