...
`caption_model` and the caption task are application wiring: the `md-image-captioning` task link in `app_model_task_config` ([appendix-a-data-model.md](./appendix-a-data-model.md)). The `embedding` task link drives `embedding_model`. Both identifiers are `{providerId}::{modelId}`.
